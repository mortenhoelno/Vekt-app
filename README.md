...
### Distribusjon til Vercel (Expo web)
Vercel kan bygge den statiske web-varianten automatisk når du peker mot dette repoet:

1. Sett **Build Command** til `npm install && npm run export:web` i Vercel-prosjektet.
2. Sett **Output Directory** til `dist`.
3. Deploy – `vercel.json` sørger for at alle ruter peker tilbake til `index.html`, slik at Expo Router fungerer på nett.

For lokale tester kan du kjøre eksporten manuelt:
